...
map<int, vector<pair<int,DnaString>>> approxFoundMatches;
...
void approximativeSearch(int num_of_seqs, int motif_length) {

	auto delegaten = [](auto & iter, DnaString const & needle, uint8_t errors) {
		for (auto occ : getOccurrences(iter))
			addToMap<pair<int,DnaString>>(approxFoundMatches, seqidx, pair<int,DnaString>(occ,substr(sequences[seqidx], occ, l)));
	};

	int c = 0;
	for (auto sequence: sequences){
		c++;
		cout << "the sequence is " << c << endl;
		for (int i = 0; i < length(sequence) - motif_length + 1; ++i){
			DnaString pattern = substr(sequence, i, motif_length);
			for(seqidx=0; seqidx<num_of_seqs; seqidx++) {
				Index<DnaString, BidirectionalIndex<FMIndex<> > > index(sequences[seqidx]);
				switch (d) {
					case 1: find<0,1>(delegaten, index, pattern, HammingDistance()); break;
					case 2: find<0,2>(delegaten, index, pattern, HammingDistance()); break;
					case 3: find<0,3>(delegaten, index, pattern, HammingDistance()); break;
					case 4: find<0,4>(delegaten, index, pattern, HammingDistance()); break;
				}
			}
		}
	}
...
void initWh(int& motif_length, pair<int,vector<pair<int,int>>>& bucket, vector<vector<float>>& Wh) {
    ...
    for(auto p : bucket.second) { //for each element in the bucket
        ...
        for (int u = 0; u < motif_length; u++) //calculate the frequencies of the bases at every position u in the l-mers
            Wh[ordValue(sequences[i][j+u])][u]++; //ordValue turns a nt into an int
    }
...
void printMatches() {
	for(auto p : approxFoundMatches) { // print out the foundMatches
		cout << p.first << ": { ";
		for(pair<int,DnaString> pos : p.second)
			cout << "[" << pos.first << " => " << pos.second << "] ";
		cout << "}" << endl;
...
int main(int argc, char const ** argv) {
    ...
    for(int i = 1; i<=datasets; i++) {
		...
		approximativeSearch(num_of_seqs, l);
		printMatches();
		exit(EXIT_FAILURE);
