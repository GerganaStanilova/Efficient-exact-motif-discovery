...
vector<uint8_t> getGenmapFrequency(string filename, int motif_length, int mismatches) {
	vector<uint8_t> frequency_vector;
	string output_folder_name = "_output_";
	string genmap_command = 
		"genmap index -F " + filename+".fasta" + " -I ./_indeces_;"
		+ " genmap map "
	...
	load(frequency_vector, "./_output_/"+filename+".txt");
...
vector<pair<int, int>> processGenmapFrequencyVector(vector<uint8_t> frequency_vector, int no_of_sequences, int sequence_length) {
    ...
    for(int i = 1, max = 0, pos = 0; i <= length(frequency_vector); i++) {
        if(max < frequency_vector[i]) {
            max = frequency_vector[i];
            pos = (i % sequence_length) - 1;
        }
...
int main(int argc, char const ** argv) {
	...
	else { cerr << "Parameters file not found or not readable"; return -1; }
    ...
    for(int i = 1; i<=datasets; i++) {
		...
		if(argc>3) {
			...
			else { cerr << "Planted motif file not found or not readable"; return -1; }
		}
